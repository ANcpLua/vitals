...
build/swift/release/claude-selftest                 # telemetry parsing, alerts, budget forecast, transcripts
build/swift/release/mcp-selftest
./install.sh                                        # build, copy to ~/Applications, (re)start the agent, register the hook
build/swift/release/vitals snapshot|claude|budget|keys|mcp|awake   # headless views, no menu needed
```
...
| `VitalsClaude` | `~/.claude` and Anthropic endpoints: status, usage, sessions, transcripts, `ClaudeBudget`, `BudgetWarning`. |
...
Pattern for every feature: a pure `decide`/`forecast`/`adding` function in Core
...
- **Budget** (`ClaudeBudget.swift`): one sample per row per poll, forecast over
  15 min needs 4 min of span, a drop inside the window is a reset. Warning only
  when a limit runs out before its reset. Per session the transcript
  ...
  by price-weighted tokens, never raw tokens per minute. Warning file
  `~/.config/vitals/budget-warning.json`; hook `hooks/budget-warning.sh` is copied
  into the bundle and registered by `install.sh` as PreToolUse in
  `~/.claude/settings.json`; it reads hook stdin before the heredoc, exits 0 always.
...
- Don't run `vitals claude` or `vitals budget` in a loop: each call spends one of
  the usage endpoint's requests and the menu bar's next poll gets a 429.
